...
import { ArrowLeft, User, TrendingUp, ShieldCheck } from "lucide-react";
      ...
      <div className="login-form-side">
        ...
        <div className="login-card">
          <div style={{ marginBottom: 32, textAlign: "center" }}>
            <div style={{ width: 72, height: 72, borderRadius: "50%", background: "linear-gradient(135deg, #D4AF37, #f0c75e)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
              <User size={36} color="#fff" />
            </div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: "#111", margin: "0 0 4px" }}>Customer Login</h1>
            <p style={{ fontSize: 14, color: "#64748b", margin: 0 }}>{COMPANY.name}</p>
          </div>
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 18 }}>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 7 }}>Customer ID</label>
              <input type="text" value={userId} onChange={(e) => setUserId(e.target.value)} placeholder="e.g. 2026CM01" required style={inputStyle} onFocus={(e) => { e.target.style.borderColor = "#D4AF37"; e.target.style.boxShadow = "0 0 0 3px rgba(212,175,55,0.1)"; }} onBlur={(e) => { e.target.style.borderColor = "#d1d5db"; e.target.style.boxShadow = "none"; }} />
            ...
            <div style={{ marginBottom: 22 }}>
              ...
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="DDMMYYYY (e.g. 15051995)" required style={inputStyle} onFocus={(e) => { e.target.style.borderColor = "#D4AF37"; e.target.style.boxShadow = "0 0 0 3px rgba(212,175,55,0.1)"; }} onBlur={(e) => { e.target.style.borderColor = "#d1d5db"; e.target.style.boxShadow = "none"; }} />
            </div>
            ...
            <button type="submit" disabled={loading} style={{ width: "100%", padding: 14, background: loading ? "#9ca3af" : "linear-gradient(135deg, #D4AF37, #f0c75e)", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", boxShadow: loading ? "none" : "0 4px 14px rgba(212,175,55,0.3)" }}>
              {loading ? "Signing in..." : "Sign In as Customer"}
            </button>
          </form>
          <div style={{ marginTop: 20, textAlign: "center" }}>
            <button onClick={() => navigate("/login")} style={{ background: "none", border: "none", color: "#1565C0", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Admin Login</button>
            <span style={{ color: "#94a3b8", margin: "0 8px", fontSize: 13 }}>|</span>
            <button onClick={() => navigate("/agent-login")} style={{ background: "none", border: "none", color: "#059669", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Agent Login</button>
          </div>
