...
export const ContactForm = () => {
  const [form, setForm] = useState({ name: '', email: '', phone: '', plan: 'other', message: '' });
    ...
    fetch('/api/schemes')
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          setSchemes(data.filter(s => s.status === 'Active'));
        }
      })
      .catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setDone(true);
        setForm({ name: '', email: '', phone: '', plan: 'other', message: '' });
      } else {
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select value={form.plan} onChange={set('plan')} className="input-base">
                      {schemes.length > 0 ? schemes.map(s => (
                        <option key={s.id} value={s.name}>{s.name} — {fmt(s.amount)} Pool</option>
                      )) : (
                        <>
